...
1. Explain the difference between `RDBMS` and `SQL`.
  ...
  RDBMS is a relational database management system software, which manages storage, querying, updating, etc...
  SQL is the language that is used to interact with the system(RDBMS).

2. Why do tables need a `primary key`?

  It ensures row level accessibility. Easy selection.
...
4. What do we need in order to have a _many to many_ relationship between two tables.
  ...
  A third table, a join table which uses for keys to reference the other two.
